...
// Temporary utility method for injecting the `useEsbuild` parameter into every
// test.
//
// @todo Replace with a more robust solution.
const zipNode = function (t, fixture, parameters = {}) {
  return zipNodeOriginal(t, fixture, { ...parameters, bundler: 'esbuild' })
}
...
test('Include most files from node modules present in `externalModules`', async (t) => {
  const { tmpDir } = await zipNode(t, 'node-module-included', { opts: { externalModules: ['test'] } })
...
test.failing('Includes specific Next.js dependencies when using next-on-netlify', async (t) => {
...
test.failing('Includes all Next.js dependencies when not using next-on-netlify', async (t) => {
...
test.failing('Throws on missing critters dependency for Next.js 9', async (t) => {
  await t.throwsAsync(zipNode(t, 'node-module-next9-critters'))
})

test.failing('Ignore missing critters dependency for Next.js 10', async (t) => {
  await zipNode(t, 'node-module-next10-critters')
})

test.failing('Ignore missing critters dependency for Next.js exact version 10.0.5', async (t) => {
  await zipNode(t, 'node-module-next10-critters-exact')
})

test.failing('Ignore missing critters dependency for Next.js with range ^10.0.5', async (t) => {
  await zipNode(t, 'node-module-next10-critters-10.0.5-range')
})

test.failing(`Ignore missing critters dependency for Next.js with version='latest'`, async (t) => {
  await zipNode(t, 'node-module-next10-critters-latest')
})
...
test('Throws on invalid package.json', async (t) => {
  ...
  try {
    await t.throwsAsync(zipNode(t, 'invalid-package-json'), /package.json:1:1: error: Expected string but found "{"/)
  } finally {
